...
Copy-Item release/$target/arkwallet.exe     release/wininstaller/
...
light.exe -ext WixUIExtension -cultures:en-us release/wininstaller/zec-qt-wallet.wixobj -out release/wininstaller/arkwallet.msi 
if (!$?) {
    exit 1;
}

New-Item artifacts -itemtype directory -Force | Out-Null
Copy-Item release/wininstaller/arkwallet.msi ./artifacts/Windows-installer-$target.msi